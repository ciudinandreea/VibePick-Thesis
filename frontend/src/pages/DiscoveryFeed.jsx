...
function MovieModal({ movieId, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      padding: 24, animation: 'md-in 0.2s ease both',
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {loading ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center',
            ...
            <span style={{ fontSize:14, color:'rgba(255,255,255,0.45)', fontWeight:600 }}>Loading…</span>
          </div>
        ) : movie ? (
          <div style={{ display:'flex', minHeight:0 }}>
            ...
            <div style={{ flex:1, padding:'26px 26px 26px 22px', minWidth:0 }}>
              <div style={{ fontSize:25, fontWeight:900, color:'white',
                letterSpacing:'-0.4px', lineHeight:1.2, marginBottom:4, paddingRight:30 }}>
                {movie.title}
              </div>
              {movie.tagline && (
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:14 }}>
                <div style={{ display:'flex', alignItems:'center', gap:5,
                  ...
                  borderRadius:8, padding:'4px 10px', fontSize:14, fontWeight:800, color:'#fcd34d' }}>
                ...
                {movie.status && (
                  <span style={{ background:'rgba(168,85,247,0.18)', border:'1px solid rgba(168,85,247,0.28)',
                    borderRadius:20, padding:'3px 10px', fontSize:12, fontWeight:700, color:'#c084fc' }}>
                    {movie.status}
                  </span>
                )}
              </div>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:16 }}>
                  {movie.genres.map(g => (
                    <span key={g.id} style={{ background:'rgba(255,255,255,0.07)',
                      ...
                      padding:'4px 11px', fontSize:12, fontWeight:700, color:'rgba(255,255,255,0.75)' }}>
                      {g.name}
                    </span>
                  ))}
                </div>
              ...
              {movie.vote_count > 0 && (
                <div style={{ marginBottom:20 }}>
                  <div style={{ fontSize:10, fontWeight:800, color:'rgba(255,255,255,0.35)',
                    textTransform:'uppercase', letterSpacing:'1px', marginBottom:4 }}>Votes</div>
              ...
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                {!saved ? (
                  ...
                  <div style={{ display:'inline-flex', alignItems:'center', gap:6,
                    ...
                    borderRadius:20, padding:'7px 14px', fontSize:13, fontWeight:700, color:'#c084fc' }}>
                    <HeartIco /> Saved!
                  </div>
                )}
                {!watched ? (
                  <button onClick={handleMarkWatched} style={{
                    ...
                    fontSize:13, fontWeight:700, color:'white', cursor:'pointer', fontFamily:FONT,
                    boxShadow:'0 4px 14px rgba(124,58,237,0.45)', transition:'opacity 0.15s',
                  }}
...
export default function DiscoveryFeed() {
  ...
  async function loadRecommendations(mode) {
    try {
      setLoading(true); setError(''); setFeedOpen(false);
      const resp = await api.get(`/recommendations?mood=${currentMood}&mode=${mode}`);
      const recs = resp.data.recommendations || [];
      setMovies(recs);
      setFeedMode(mode);
      setFeedTitle(
        mode === 'mood-aware'
          ? `Mood-Aware Recommendations (${currentMood})`
        ...
        <div style={{ padding:'28px 32px 56px' }}>
          ...
          {feedMode && (
            <div style={{
              ...
              background: feedMode === 'mood-aware'
                ? 'rgba(124,58,237,0.1)' : 'rgba(107,92,126,0.1)',
              border: `1px solid ${feedMode === 'mood-aware' ? 'rgba(124,58,237,0.25)' : 'rgba(107,92,126,0.2)'}`,
              borderRadius:24, padding:'6px 14px', marginBottom:16,
              fontSize:12, fontWeight:700,
              color: feedMode === 'mood-aware' ? PUR : MUT,
            }}>
              {feedMode === 'mood-aware' ? <MoodIco /> : <BaselineIco />}
              {feedMode === 'mood-aware' ? `Mood-Aware · feeling ${currentMood}` : 'Baseline mode'}
              <button onClick={loadPopular} style={{
                background:'none', border:'none', cursor:'pointer',
                color:'inherit', padding:0, fontFamily:FONT,
          ...
          {loading && (
            <div style={{ display:'flex', flexDirection:'column',
              alignItems:'center', justifyContent:'center', minHeight:320, gap:14 }}>
          ...
          {!loading && error && (
            <div style={{ background:'rgba(239,68,68,0.07)',
              ...
              {error}
              <button onClick={loadPopular} style={{ background:'none', border:'none',
                color:PUR, fontSize:13, fontWeight:700, textDecoration:'underline',
                cursor:'pointer', fontFamily:FONT }}>Try again</button>
            </div>
          )}
